...
window.Vaadin.Flow['_vaadintheme_newsappweb_globalCss'] = window.Vaadin.Flow['_vaadintheme_newsappweb_globalCss'] || [];
export const applyTheme = (target) => {
  
  const injectGlobal = (window.Vaadin.Flow['_vaadintheme_newsappweb_globalCss'].length === 0) || (!window.Vaadin.Flow['_vaadintheme_newsappweb_globalCss'].includes(target) && target !== document);
  if (injectGlobal) {
    injectGlobalCss(stylesCss.toString(), target);
    
    window.Vaadin.Flow['_vaadintheme_newsappweb_globalCss'].push(target);
  }
  if (!document['_vaadintheme_newsappweb_componentCss']) {
    
    document['_vaadintheme_newsappweb_componentCss'] = true;
  }
